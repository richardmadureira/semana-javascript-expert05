...
import pretryBytes from 'pretty-bytes';
...
export default class FileHelper {
    static async getFilesStatus(downloadsFolder) {
        ...
        for(const fileIndex in currentFiles){
            ...
            filesStatuses.push({size: prettyBytes(size), file: currentFiles[fileIndex], lastModified: birthtime, owner: process.env.USER})
        }
